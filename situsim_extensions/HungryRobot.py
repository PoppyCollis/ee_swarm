...
class HungryRobot(Robot):
    # construct robot
    # todo i have changed decayrate2 from 0.01 and decayrate from 1; was 0.05 and 0.01
    def __init__(self, id, x, y, controller,
                 ...
                 decay_rate=0.0, decay_rate2=0.0,
    ...
    def step(self, dt):
        ...
        # todo STUCK DETECTOR
        # print(self.id, self.energies[-1])
        self.internal_time +=dt

        if self.energies[-1] > 1200: #todo this needs to not overwrite the disturbance
            self.decay_rate = 2
        else:
        ...
        # print(self.timer)
        # this detects whether a collision has occurred with another robot in the simulation
        # initiates horizontal gene transfer
        # todo check gene transfer is actually working - BOTH WAYS
        mut_rate = 2
        if self.energies[-1] < 9900: # todo relate this to the fitness (gradient of energy) instead
            mut_rate = 6
        ...
        if self.timer > 0 and self.timer < 150:
            ...
            for agent in self.swarm:
                if np.linalg.norm([self.x - agent.x, self.y - agent.y]) < self.radius*2:
                    self.fitness = sigmoid2(self.energies[-1])
                    # print(self.fitness)
                    if self.id < agent.id:
                        # do nothing because the other robot will deal with the gene transfer process
                        pass
                    else:
                        # print("COLLISION!")
                        if agent.energies[-1] > self.energies[-1]:
                                # print("before:", self.controller.weights, agent.controller.weights)
                                # print("other robot wins")
                            num = random.randint(0,100)
                            if num > 90:
                                self.controller.weights = agent.controller.weights.copy()
                                # mutate
                                gene = np.random.randint(0,len(agent.controller.weights))
                                # simple version is just pick another number between -8,7
                                # self.controller.weights[gene] = np.random.randint(-8, 7)
                                # alternatively can just change one of the values by 1
                                self.controller.weights[gene] = self.controller.weights[gene] + random.randint(-mut_rate,mut_rate)
                                if self.controller.weights[gene] > 7:
                                    self.controller.weights[gene] = 6
                                if self.controller.weights[gene] < -8:
                                    self.controller.weights[gene] = -7
                                # print("after:", self.controller.weights, agent.controller.weights)
                                self.timer = self.timer + 1
                                # print(self.timer)
                        else:
                            num = random.randint(0, 100)
                            if num > 90:
                                # print('this robot wins')
                                # print("before:", agent.controller.weights, self.controller.weights)
                                agent.controller.weights = self.controller.weights.copy()
                                # mutate
                                gene = np.random.randint(0, len(agent.controller.weights))
                                # simple
                                # self.controller.weights[gene] = np.random.randint(-8, 7)
                                # self.controller.weights[gene] = np.random.randint(-4, 4)
                                # mutate by just 1
                                self.controller.weights[gene] = self.controller.weights[gene] + random.randint(-2,2)
                                if self.controller.weights[gene] > 7:
                                    self.controller.weights[gene] = 6
                                if self.controller.weights[gene] < -8:
                                    self.controller.weights[gene] = -7

                                # print("after:", agent.controller.weights, self.controller.weights)
                                self.timer = self.timer + 1

                # compare own fitness to the agent
                # if better, overwrite agents genes and mutate
                # if worse, be overwritten and mutate
...
class HungryController(Controller):
    ...
    def __init__(self,gain=1,
        ...
        # self.weights = list(np.random.randint(-8,7,6)) # this constitutes the entire genotype, each weight is a single gene
    ...
    def step(self, inputs, dt):
        ...
        # self.sensor_p = int(inputs[3]>inputs[2])

        self.nn_inputs = np.array([self.sensor_f, 1])  # 1 is the bias term
        # self.nn_inputs = np.array([self.sensor_f, self.sensor_p, 1])  # 1 is the bias term

        self.left_speed_command = sigmoid(np.dot(np.array(self.weights[:2]).T, self.nn_inputs))
        # self.left_speed_command = sigmoid(np.dot(np.array(self.weights[:3]).T, self.nn_inputs))
        self.right_speed_command =  sigmoid(np.dot(np.array(self.weights[2:]).T, self.nn_inputs))
        # self.right_speed_command =  sigmoid(np.dot(np.array(self.weights[3:]).T, self.nn_inputs))

        # self.left_speed_command = math.tanh(np.dot(np.array(self.weights[:2]).T, self.nn_inputs))
        # self.right_speed_command =  math.tanh(np.dot(np.array(self.weights[2:]).T, self.nn_inputs))
