...
class HungryRobot(Robot):
    ...
    def get_broadcast(self):
        # gene_num = random.randint(0, len(self.controller.weights) - 1)

        mut_num = 1

        genes = random.sample(list(set([x for x in range(len(self.controller.weights))])), k=mut_num)

        broadcast = self.controller.weights.copy()
        # mutate by some amount
        broadcast[genes[0]] = broadcast[genes[0]] + random.randint(-5, 5)
        # broadcast[genes[1]] = broadcast[genes[1]] + random.randint(-5, 5)
        # broadcast[genes[2]] = broadcast[genes[1]] + random.randint(-5, 5)
        # broadcast[genes[3]] = broadcast[genes[1]] + random.randint(-5, 5)
        # broadcast[genes[2]] = broadcast[genes[1]] + random.randint(-5, 5)
        # broadcast[gene_num] = broadcast[gene_num] + random.randint(-1, 1)
        if broadcast[genes[0]] > 7:
            broadcast[genes[0]] = 6
        if broadcast[genes[0]] < -8:
            broadcast[genes[0]] = -7
        #
        # if broadcast[genes[1]] > 7:
        #     broadcast[genes[1]] = 6
        # if broadcast[genes[1]] < -8:
        #     broadcast[genes[1]] = -7
        #
        # if broadcast[genes[2]] > 7:
        #     broadcast[genes[2]] = 6
        # if broadcast[genes[2]] < -8:
        #     broadcast[genes[2]] = -7
        #
        # if broadcast[genes[3]] > 7:
        #     broadcast[genes[3]] = 6
        # if broadcast[genes[3]] < -8:
        #     broadcast[genes[3]] = -7
        ...
        for consumable in self.consumables:
            if np.linalg.norm([self.x - consumable.x, self.y - consumable.y]) < consumable.radius:
                ...
                if consumable.real_type == Consumables.food:
                    ...
                    self.energy -= quantity
                # resets robot to random starting position
        ...
        if self.timer > 0 and self.timer < 150:
            ...
            if self.timer > 0 and self.timer < 150:
                ...
                for agent in self.swarm:
                    ...
                    # constrain in y
                    # # todo using arena code, try to constrain agents from overlapping each other
                    # if (self.state[1] + self.radius) == (agent.state[1] - agent.radius):
                    #     print('ahh')
                    #     self.state[1] = self.state[1] - 5
                    # elif (self.state[1] - self.radius) == (agent.state[1] + agent.radius):
                    #     self.state[1] = self.state[1] + 5
                    #
                    # # constrain in x
                    # if (self.state[0] + self.radius) == (agent.state[0] - agent.radius):
                    #     self.state[0] = self.state[0] - 5
                    # elif (self.state[0] - self.radius) == (agent.state[0] + agent.radius):
                    #     self.state[0] = self.state[0] + 5
...
class HungryController(Controller):
    ...
    def step(self, inputs, dt):
        ...
        # self.left_speed_command = np.dot(np.array(self.weights[:2]).T, self.nn_inputs)
        # self.right_speed_command = np.dot(np.array(self.weights[2:]).T, self.nn_inputs)
